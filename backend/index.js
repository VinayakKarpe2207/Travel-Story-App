...
app.post("/create-account", async (req, res) => {
  ...
  if (!fullName || !email || !password) {
    return res
    .status(400)
    .json({ erro: true, message: "All fields are required" });
  }
...
app.post("/image-upload", upload.single("image"), async (req, res) => {
 ...
 app.delete("/delete-image", async (req, res) => {
  ...
  if (!imageUrl) {
   return res.status(400).json({ erro: true, message: "imageUrl parameter is required" });
  }
...
app.put("/edit-story/:id", authenticateToken, async (req, res) => {
  ...
  if (!title || !story || !visitedLocation || !visitedDate) {
   ...
   try {
    // Find the travel story by ID and ensure it belong to authenticate user
    ...
    const placeholderImgUrl = `http://localhost:8000/assest/placeholder.png`;
    ...
    await travelStory.save();
    res.status(200).json({ story: travelStory, message:'Update Successdull' });
   } catch (error) {
    res.status(500).json({ error: true, message: error.message });
   }
});
